						...
						<div className="space-y-4">
							...
							<div className="rounded-lg border border-copper/20 bg-charcoal/40 p-4">
								...
								<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
									...
									/>
									<input
										value={orderDetails.pantsSize}
										onChange={(event) => updateOrderDetail('pantsSize', event.target.value)}
										placeholder="Pants size"
										className="w-full rounded-lg border border-copper/30 bg-charcoal/70 px-4 py-3 text-cream placeholder:text-beige/70 focus:outline-none focus:border-copper"
									/>
									<input
										value={orderDetails.foldHole}
										onChange={(event) => updateOrderDetail('foldHole', event.target.value)}
										placeholder="Fold-hole"
										className="w-full rounded-lg border border-copper/30 bg-charcoal/70 px-4 py-3 text-cream placeholder:text-beige/70 focus:outline-none focus:border-copper"
									/>
									<input
										value={orderDetails.purseSize}
										onChange={(event) => updateOrderDetail('purseSize', event.target.value)}
										placeholder="Purse size"
										className="w-full rounded-lg border border-copper/30 bg-charcoal/70 px-4 py-3 text-cream placeholder:text-beige/70 focus:outline-none focus:border-copper"
									/>
									<input
										value={orderDetails.leatherMaterial}
										onChange={(event) => updateOrderDetail('leatherMaterial', event.target.value)}
										placeholder="Leather / material"
										className="w-full rounded-lg border border-copper/30 bg-charcoal/70 px-4 py-3 text-cream placeholder:text-beige/70 focus:outline-none focus:border-copper"
									/>
									<input
										value={orderDetails.hardware}
										onChange={(event) => updateOrderDetail('hardware', event.target.value)}
								...
								<div className="space-y-3 mt-3">
									<input
										value={orderDetails.toolingDesign}
										onChange={(event) => updateOrderDetail('toolingDesign', event.target.value)}
										placeholder="Tooling / design"
										className="w-full rounded-lg border border-copper/30 bg-charcoal/70 px-4 py-3 text-cream placeholder:text-beige/70 focus:outline-none focus:border-copper"
									/>
									<input
										value={orderDetails.laceStitching}
										onChange={(event) => updateOrderDetail('laceStitching', event.target.value)}
										placeholder="Lace / stitching"
										className="w-full rounded-lg border border-copper/30 bg-charcoal/70 px-4 py-3 text-cream placeholder:text-beige/70 focus:outline-none focus:border-copper"
									/>
									<input
										value={orderDetails.specialFinish}
										onChange={(event) => updateOrderDetail('specialFinish', event.target.value)}
										placeholder="Pipeliner / special finish"
										className="w-full rounded-lg border border-copper/30 bg-charcoal/70 px-4 py-3 text-cream placeholder:text-beige/70 focus:outline-none focus:border-copper"
									/>
								</div>
							</div>
